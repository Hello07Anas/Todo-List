...
- (void)viewWillAppear:(BOOL)animated {
    ...
    NSInteger index = 0;
        while (index < taskArray.count) {
            if (![taskArray[index].status isEqual:@"Done"]) {
...
- (void)viewDidLoad {
    ...
    [self.tableViewDone  reloadData];
    _todoImg.image = [UIImage imageNamed:@"todoImgDone"];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    NSPredicate *donePredicate = [NSPredicate predicateWithFormat:@"status == %@", @"Done"];
    NSArray<Task *> *doneTasks = [taskArray filteredArrayUsingPredicate:donePredicate];
    
    if (doneTasks.count == 0) {
        _tableViewDone.hidden = true;
        _todoImg.hidden = false;
    } else {
        _tableViewDone.hidden = false;
...
- (void)deleteTaskAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [_tableViewDone deleteRowsAtIndexPaths:@[indexPath] withRowAnimation:UITableViewRowAnimationFade];
